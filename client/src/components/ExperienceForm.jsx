                ...
                <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                    ...
                    <div className='space-y-2'>
                        <div className="flex items-center justify-between">
                            ...
                            <button className="flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 hover:bg-purple-200 transition-colors disabled:opacity-50" > 
                                <Sparkles className='w-3 h-3'/>
                                Enhance with AI
                            </button>
                        </div>
                        <textarea value={experience.description || ""} onChange={(e) =>  updateExperience(index, "description", e.target.value)} rows={4} className="w-full text-sm px-3 py-2 rounded-lg resize-none border outline-none focus: focus:ring-blue-500 focus:border-blue-500 transition-colors" placeholder="Describe your key responsibilities achievements..."/>
                    </div>
                </div>
               ))}
            </div>
